...
<body class="bg-slate-100 dark:bg-slate-900 transition-colors duration-200">
    ...
    <div id="trashBinModal" class="fixed inset-0 hidden z-50 flex items-center justify-center">
        ...
        <div class="relative bg-white dark:bg-slate-800 rounded-xl max-w-lg w-full mx-4 shadow-2xl">
            <div class="px-4 py-3 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
                <div class="flex items-center justify-between">
                    <h3 class="text-lg font-medium text-slate-900 dark:text-white">Bin Status Notifications</h3>
                ...
                <div class="mt-6">
                   <div class="space-y-3">
                        @foreach($notifications as $notification)
                            <div class="p-3 rounded-lg border-l-4 border-red-500 bg-emerald-50 dark:bg-emerald-900/30"
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-3">
                                        <p class="text-sm text-slate-900 dark:text-white">
                                            {{ ucfirst($notification['bin_type']) }} bin needs attention
                                        </p>
    ...
    <div class="fixed bottom-4 right-4 z-50">
        <div class="bg-white dark:bg-slate-800 rounded-lg shadow-lg p-4 border-l-4 border-blue-500">
            <div class="flex items-center">
                <div class="flex-shrink-0">
                ...
                <div class="ml-3">
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">
                        Last updated: {{ session('notification.time') }}
                    </p>
                </div>
                <div class="ml-4">
...
<script>
    ...
    function markAsRead(notificationId) {
        ...
        notification.classList.remove('bg-emerald-50', 'dark:bg-emerald-900/30');
        
        // Store clicked state in localStorage
        localStorage.setItem(`notification-${notificationId}-clicked`, 'true');
    }

    // Add this after markAsRead function
    document.addEventListener('DOMContentLoaded', function() {
        ...
        notifications.forEach(notification => {
            ...
            if (localStorage.getItem(`notification-${id}-clicked`) === 'true') {
                notification.classList.remove('bg-emerald-50', 'dark:bg-emerald-900/30');
            }
        });
    });
...
<script>
    Echo.channel('bin-status')
        .listen('BinStatusUpdated', (e) => {
            updateBinStatus(e.binStatus);
            updateNotificationCount(e.notificationCount);
        });

    function updateBinStatus(binStatus) {
        const modalContent = document.getElementById('binStatusContent');
        if (!modalContent) return;

        // Only show notifications
        const notificationsHtml = Array.isArray(binStatus.notifications) ? 
            createNotificationsHtml(binStatus.notifications) : '';
    ...
    function createNotificationsHtml(notifications) {
            ...
            <div class="mt-6">
                <div class="space-y-3">
                    ${notifications.map(notification => {
                        const isClicked = localStorage.getItem(`notification-${notification.id}-clicked`) === 'true';
                        const bgClass = isClicked ? '' : 'bg-emerald-50 dark:bg-emerald-900/30';
                        
                        return `
                            <div class="p-3 rounded-lg border-l-4 border-red-500 ${bgClass}" 
                                 id="notification-${notification.id}"
                                 onclick="markAsRead('${notification.id}')">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0">
                                    ...
                                    <div class="ml-3">
                                        <p class="text-sm text-slate-900 dark:text-white">
                                            ${notification.bin_type.charAt(0).toUpperCase() + notification.bin_type.slice(1)} bin needs attention
                                        </p>
                                        <p class="text-xs text-slate-500 dark:text-slate-400">
